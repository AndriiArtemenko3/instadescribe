...
function packedArchive(args, label) {
  const result = JSON.parse(command(npm, ["pack", ...args, "--pack-destination", packs, "--json"]));
...
try {
  ...
  const installedEntry = join(consumer, "node_modules", "instadescribe", "dist", "bin.js");
  ...
  const cliVersion = command(installedBin, ["--version"], { cwd: consumer });
