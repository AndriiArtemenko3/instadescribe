# InstaDescribe v0.1 Cloud Core — legacy release evidence

> This release was created under the former product name **InstaScribe**. The
> current product name is **InstaDescribe**. Private-only commit/tag identities and
> PII-bearing screenshots are excluded from this curated public snapshot.

**Gate status:** passed and released

**Historical source identity:** verified in the private release record. Exact
private-only tag and commit identifiers are intentionally omitted because they are
not part of the curated public ancestry.

**Region:** `eu-west-2`

**Published static frontend:** [Cloud Core](https://dax591xorok0e.cloudfront.net/)

**Dated API-readiness observation:** unavailable on `2026-08-29`
([readiness endpoint](https://dax591xorok0e.cloudfront.net/api/readyz))

> **Availability observation captured on `2026-08-29`:** the static frontend
> returned HTTP 200, while the API readiness endpoint returned HTTP 503 with its
> database check unavailable. The successful deployment and health results below
> are explicitly historical measurements captured for the v0.1 release; this packet
> makes no claim about present availability or support.

## Release verdict

The v0.1 Cloud Core milestone passed. At release acceptance, InstaScribe's existing
multimodal pipeline was deployed as a bounded AWS application with direct private
uploads, durable relational state, queued container processing, manifest-driven
artifact delivery and persisted human edits. One single-attempt GPT-4.1 validation
on a rights-cleared 60-second clip reached `READY_FOR_REVIEW` and rendered five scenes
in the then-live editor.

At the recorded release closeout, the environment was deliberately cost-controlled:
the API was at one task, the 2-vCPU/8-GiB worker was at zero, both task definitions
were in deterministic fake mode and no deployed task could read the stored OpenAI
value. The dated availability observation is stated separately above.

This is a portfolio cloud release, not a claim of production-grade SaaS operation.

## Release-time deployed architecture
...
The raw operator transcript and account-scoped smoke log are intentionally excluded
from the public snapshot; the bounded results are reproduced above.
...
At the standard GPT-4.1 prices checked on 2026-08-10, the measured token counts implied
an estimated **$0.03443** model charge if all input was billed uncached. This is not
an invoice, current price quote or total historical OpenAI spend. The account-scoped
provider transcript is intentionally excluded from the public snapshot.
...
The release-time registry audit remained nonzero. That dated result is historical;
the current candidate must pass the dependency gate described in
[PUBLIC_SNAPSHOT.md](../../PUBLIC_SNAPSHOT.md) before publication.
...
recorded as having been configured as an alert, not a spending cap. Actual AWS
invoice totals are not claimed.

## Historical final safe operating posture

A release-time read-only audit after evidence capture established:
...
At release closeout, the OpenAI secret value was stored out of band and inaccessible
to the deployed fake-mode tasks.

## Historical screenshots intentionally omitted

The original v0.1 dashboard and editor captures contained an operator identity,
validation titles and real job history. They are excluded from the curated public
snapshot instead of being blurred or presented as synthetic evidence. The current
repository-level product capture uses only the committed Sintel fixture and is not
represented as a screenshot of this legacy deployment.

Historical demo sequence used while the API was healthy:
